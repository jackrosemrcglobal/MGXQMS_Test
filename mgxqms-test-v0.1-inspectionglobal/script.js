...
function initSimpleSaveAndLoad() {
  ...
  function saveFormData(reportName) {
    ...
    // Convert FormData to object, handling checkbox arrays properly
    for (let [key, value] of formData.entries()) {
      // Skip file inputs
      if (key !== 'file-attachments') {
        // Handle checkbox arrays (names ending with [])
        if (key.endsWith('[]')) {
          const baseKey = key.slice(0, -2); // Remove the []
          if (!data[baseKey]) {
            data[baseKey] = [];
          }
          data[baseKey].push(value);
        } else {
          // Handle single values, but check if we already have this key
          if (data[key] !== undefined) {
            // Convert to array if we have multiple values for the same key
...
function initImageDocumentation() {
  ...
  fileInput.addEventListener('change', (e) => {
    ...
    if (files.length > 0) {
      // Remove placeholder if it exists, before adding new images
      const placeholder = imagesContainer.querySelector('.no-image-text');
      if (placeholder) {
        placeholder.remove();
      }
            ...
            if (nameParts.length > 1 && (nameParts[nameParts.length - 1] === 'template' || !isNaN(nameParts[nameParts.length - 1]))) {
                // Find the current highest index for the 'select' name across this table
                 ...
                 table.querySelectorAll('tbody tr').forEach(existingRow => {
                   ...
                   const newIndex = highestIndex + 1;
                    let newName = nameParts.slice(0, -1).join('-') + '-' + newIndex.toString();
                    ...
                    if (nameParts.length > 2 && nameParts[nameParts.length - 2] === 'template' && nameParts[nameParts.length - 1] === '2') {
                        ...
                        newName = nameParts.slice(0, -1).join('-') + '-' + newIndex.toString(); // doc-num-21, doc-title-21
